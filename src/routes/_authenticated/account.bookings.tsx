...
export const Route = createFileRoute("/_authenticated/account/bookings")({
  ...
  validateSearch: (s) =>
    z.object({ highlight: z.string().uuid().optional() }).parse(s),
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {bookings.map((b) => {
              const venue = b.venues as { name?: string; address_data?: unknown; cover_image_url?: string | null } | null;
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-lead/60 mt-1">{formatDateRange(b.start_time, b.end_time)}</p>
                    <div className="flex items-center gap-3 mt-2">
                      <StatusBadge status={b.status} />
                      <span className="text-sm font-medium">{formatMoney(b.total_cents, b.currency)}</span>
                    </div>
                  </div>
...
function StatusBadge({ status }: { status: string }) {
  const color =
    ...
    status === "cancelled" ? "bg-stone-100 text-stone-500" :
    "bg-red-100 text-red-700";
  return <span className={`inline-block px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded ${color}`}>{status}</span>;
}
